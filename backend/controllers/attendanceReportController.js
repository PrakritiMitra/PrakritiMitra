...
exports.generateAttendanceReport = async (req, res) => {
  try {
    ...
    const organizerData = event.organizerTeam.map(obj => ({
      ...
      role: obj.user._id.toString() === event.createdBy._id.toString() ? 'Creator' : 'Organizer',
...
const generateExcelReport = async (data, res) => {
  try {
    ...
    // Add metadata
    workbook.creator = 'PrakritiMitra';
    ...
    data.volunteers.forEach(vol => {
      const row = volunteersSheet.addRow([
        ...
        new Date(vol.registrationDate).toLocaleDateString()
      ]);
...
const generateCSVReport = async (data, res) => {
  try {
    ...
    // Add header
    csvContent += 'PrakritiMitra - Attendance Report\n';
    ...
    // Add footer
    csvContent += '\n';
    csvContent += 'Report generated by PrakritiMitra\n';
...
const generatePDFReport = async (data, res) => {
  try {
    ...
    doc
      ...
      .fontSize(16)
      .fill(colors.white)
      .text('PrakritiMitra - Attendance Report', 25, 15);
    ...
    doc
      ...
      .fontSize(8)
      .fill(colors.white)
      .text(`Generated: ${new Date().toLocaleDateString()} by ${data.generatedBy}`, pageWidth - 200, 20, { align: 'right' });
    ...
    const addPageIfNeeded = (requiredHeight) => {
      if (y + requiredHeight > pageHeight - 50) {
        ...
        doc
          ...
          .fontSize(16)
          .fill(colors.white)
          .text('PrakritiMitra - Attendance Report (Continued)', 25, 15);
    ...
    doc
      ...
      .fill(colors.secondary)
      .text(
        `PrakritiMitra - Empowering Community Service`,
        25,
        footerY,
