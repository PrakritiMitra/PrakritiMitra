...
export default function VolunteerForm() {
  ...
  const handleGoogleOAuth = async (token) => {
    ...
    try {
      ...
      if (error.response?.data?.errorType === 'RECENTLY_DELETED_ACCOUNT') {
        console.log('🗑️ VolunteerForm: Recently deleted account error detected in OAuth');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('🎉 Volunteer account created successfully! Please login to continue.');
